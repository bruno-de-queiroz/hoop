#!/bin/bash
#@module Install - install/configure the hoop CLI, and run the setup wizard
...
# side-effect-free to source, so `hoop install` / completion stay fast.
...
# =============================================================================
# hoop install setup — the interactive stack wizard (native port of /hoop:setup)
#
...
_setup_consent() {
  ...
  ${_B}hoop install setup${_RST} — configure your Claude Code stack in the sandbox.
  ...
  case "$pick" in
    ...
    Serena)
      ...
      if _setup_exec "uv tool install serena-agent" \
        ...
        _setup_note "Serena: 'uv tool install serena-agent' failed in the sandbox — check egress/network and re-run 'hoop install setup'."
      fi ;;
    Cognee)
...
_setup_dmr() {
  ...
  if ! _p_confirm "Set up Docker Model Runner on the host now?" y; then
    _setup_note "DMR: declined host setup — semantic search left BM25-only. Re-run 'hoop install setup' to enable."
    PICK_SEMANTIC="skipped"; return 1
  fi
  ...
  if [ ${#mr[@]} -eq 0 ]; then
    if command -v dmr >/dev/null 2>&1; then
      _setup_guided "The standalone 'dmr' binary is installed but no daemon is serving on :12434 — start it, then re-run 'hoop install setup':" \
        "dmr serve &        # runs the inference daemon on :12434"
    else
      local os arch
      os="$(uname -s | tr '[:upper:]' '[:lower:]')"; arch="$(uname -m)"
      case "$arch" in x86_64|amd64) arch=amd64 ;; arm64|aarch64) arch=arm64 ;; esac
      _setup_guided "Docker Model Runner isn't available. Pick ONE, then re-run 'hoop install setup':" \
    ...
    _setup_note "DMR: nothing serving on :12434 — see the printed options (Docker Desktop / Docker Engine / standalone dmr), then re-run 'hoop install setup'."
    PICK_SEMANTIC="skipped"; return 1
  fi
...
#@public ~ interactive wizard: configure the sandbox stack (native /hoop:setup)
...
function setup() {
  ...
  if [[ "${SETUP_RESET:-false}" == true ]]; then
    _setup_reset || { echo "Exited. Re-run 'hoop install setup' any time."; return 0; }
  fi
  SETUP_TEMPLATES="$HS_PLUGIN_ROOT/templates"
  SETUP_CATALOG="$HS_PLUGIN_ROOT/catalog"
  _setup_consent || { echo "Exited. Re-run 'hoop install setup' any time."; return 0; }
