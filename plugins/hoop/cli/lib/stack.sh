...
# Embedding-backend override written by `hoop install setup` when Docker Model
...
# auto-detect, `hoop doctor`, and `hoop install setup` — so the port/path lives
...
# Generated by 'hoop install setup' (Docker Model Runner) — do not edit by hand.
...
_hs_guard_dmr_override() {
  ...
  echo "  Re-enable it (docker desktop enable model-runner --tcp 12434), or run 'hoop install setup' to switch embedding backend." >&2
...
_hs_compose_guard() {
  ...
  if ! _hs_compose_supports_models; then
    ...
    echo "  Fix: upgrade Docker Compose, or run 'hoop install setup' to switch backend." >&2
    HS_DMR_OVERRIDE_ACTIVE=0
    _hs_compose_reload
  fi
  return 0
...
# Upsert KEY=VALUE into the opt-in env file (0600). Used by `hoop install setup`
...
_hs_detect_dmr() {
  ...
  if ! command -v curl >/dev/null 2>&1; then
    ...
    echo "  Configure semantic search via 'hoop install setup'."
    return 0
  fi
  if curl -fsS --connect-timeout 1 "$HS_DMR_PROBE_URL" >/dev/null 2>&1; then
    ...
    echo "  Enable semantic search via 'hoop install setup' — recommended: Docker Model Runner"
