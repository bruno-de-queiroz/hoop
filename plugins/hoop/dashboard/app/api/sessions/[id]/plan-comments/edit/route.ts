...
// Edit a comment's body. Author-scoped: the sandbox returns 403 for non-authors.
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    return Response.json(await client.editPlanComment(id, { requestId, commentId, body: text }, forwardedParticipant(req)));
  } catch (e: any) {
    return errorResponse(e?.message ?? "edit failed", typeof e?.status === "number" ? e.status : 500);
  }
}
