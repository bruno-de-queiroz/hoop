...
| Mode | Effect |
|---|---|
| `start` | `docker compose up -d --build dashboard`; opens browser when health endpoint responds. |
| `stop` | `docker compose down` for the dashboard project. |
| `restart` | stop + start. |
| `status` | reports running / not running. |
| `logs` | tails the dashboard container logs. |

---

## Execution

1. Resolve the launcher: `${CLAUDE_PLUGIN_ROOT}/dashboard/bin/hoop-dashboard`.
2. Run with the user's chosen mode (default `start`). Show stdout/stderr to the user verbatim — the script narrates its own progress.
...
- First start builds the dashboard image (~1-2 min). Subsequent starts are seconds.
...
```bash
"${CLAUDE_PLUGIN_ROOT}/dashboard/bin/hoop-dashboard" "${1:-start}"
```
