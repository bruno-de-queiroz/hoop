...
- If `hoop` is on PATH:  `hoop setup`
- Otherwise:            `"$HOOP_CLI" setup`   (or run `"$HOOP_CLI" install` first to put `hoop` on PATH)
...
Do **not** try to run `hoop setup` yourself — it will detect the
