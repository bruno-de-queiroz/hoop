...
**Live-editing the dashboard UI (`HOOP_DASHBOARD_DEV=1`).** Normally the dashboard is a baked production image, so a UI change needs `hoop dashboard rebuild`. To iterate without rebuilds, set `HOOP_DASHBOARD_DEV=1`: the CLI generates a compose override that builds the Dockerfile `dev` target (as `hoop-dashboard-dev`, leaving the prod image untouched), bind-mounts the host `dashboard/` + `shared/` source, and runs `next dev` (webpack HMR) through the same front process (`server.mjs`) — so edits reload live. First switch builds the dev image once (`HOOP_DASHBOARD_DEV=1 hoop dashboard rebuild`); thereafter `restart` is enough. It polls for file changes (`WATCHPACK_POLLING`) so it works even when the source rides a filesystem that emits no inotify events (Docker Desktop virtiofs, or a remote/sshfs docker host). Unset the var and `hoop dashboard rebuild` to return to the baked prod image. This is a dev convenience — the dev stage omits `cloudflared`, so share tunnels aren't available in dev mode. Mirrors `HOOP_PLUGIN_DEV=1`, which does the same live-overlay for the sandbox's baked `/opt/hoop` plugin source.
